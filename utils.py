...
def get_artist_roast(name):
    response = co.chat(
        ...
        directed at the playlist curator(dont refer to them in the roast) for listening to too much of {name}, their top artist. 
...
def get_genre_roast(name):
    response = co.chat(
        ...
        message=f"""
        Write one sharp, 2-3 lines max roast from Noodles (no cheesy references, and ALWAYS include Noodle's name in the roast) 
        directed at the playlist curator(dont refer to them in the roast) for having their playlist dominated by the {name} genre. 
